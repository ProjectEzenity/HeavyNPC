...
public class CmdCreateNPC implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!(sender instanceof Player)) {
            com.ezenity.heavynpc.configuration.Lang.send(sender, com.ezenity.heavynpc.configuration.Lang.PLAYER_COMMAND);
            com.ezenity.heavynpc.util.Logger.error(sender + " is not a player. Cannot create Light NPC.");
            return true;
        }
        // Make sender player
        ...
        if (!target.hasPermission("command.lightnpc.create")) {
            com.ezenity.heavynpc.configuration.Lang.send(target, com.ezenity.heavynpc.configuration.Lang.COMMAND_NO_PERMISSION
            .replace("{getCommand}", "/npc create"));
            com.ezenity.heavynpc.util.Logger.error(target + " does not have permission to use /npc create, cancelling.");
            return true;
        }

        // Change for "create" argument
        if (args.length < 2) {
            com.ezenity.heavynpc.configuration.Lang.send(target, "Please specify a Light NPC Name."); // TODO: create Lang
            com.ezenity.heavynpc.configuration.Lang.send(target, "/npc create (name)"); // TODO: Create Lang
            com.ezenity.heavynpc.util.Logger.error(target + " did not provide a light npc name, cancelling.");
            return true;
        }
        ...
        if (npcID == null) { // Statement will change once NPCManage is created
            com.ezenity.heavynpc.configuration.Lang.send(target, "Did not create Light NPC, Light NPC ID is invalid."); // TODO: Create Lang
            com.ezenity.heavynpc.util.Logger.error("Light NPC Id is null, cancelling.");
            return true;
        }
